...
/* ================= Grid, physics, scene ================= */
/* Increase the default grid resolution to improve visual fidelity on modern hardware.  */
/* Note: A larger NX/NY consumes more memory but yields smoother fields and better
 * accuracy.  600×600 is a reasonable compromise between detail and performance.  */
#define NX 600
#define NY 600
...
/* ================= Wrapped text rendering & legend panel ================= */
/*
 * Render multi-line text using SDL_ttf and wrap at the specified width.  This
 * helper builds a surface via TTF_RenderUTF8_Blended_Wrapped and converts it
 * to a texture.  The caller must free the returned texture via
 * SDL_DestroyTexture().  If outw/outh are non-NULL they receive the width
 * and height of the rendered surface.  A NULL font will return NULL.
 */
...
/*
 * Draw a wrapped legend panel at (x,y).  The lines array is concatenated
 * using newlines and wrapped to the given width (maxw).  A semi-transparent
 * dark background and light border are drawn behind the text.  Padding is
 * applied around the text.  The panel width expands to accommodate the
 * wrapped text but will not exceed maxw.
 */
static void draw_legend_bottom(SDL_Renderer* ren, TTF_Font* font,
                               int x, int y,
                               const char** lines, int nlines,
                               int maxw)
{
    /* build one long string with newlines */
    char buf[4096];
    buf[0] = '\0';
    for (int i = 0; i < nlines; ++i) {
        strncat(buf, lines[i], sizeof(buf) - strlen(buf) - 1);
        if (i < nlines - 1) strncat(buf, "\n", sizeof(buf) - strlen(buf) - 1);
    }
    SDL_Color fg = {230, 230, 230, 255};
    int tw=0, th=0;
    SDL_Texture* tex = render_text_wrapped(ren, font, buf, fg, (Uint32)maxw, &tw, &th);
    if (!tex) return;
    const int pad = 8;
...
int main(void){
    ...
    /* UI geometry */
    int scale=2;
    /* Reserve ample space for the bottom UI bar.  A taller bar avoids text
     * clipping when wrapping the legend.  If you add more legend lines
     * later, increase this value accordingly. */
    int UI_H = 160;
    const int SIM_YOFF = 0;
    /* Y coordinate for the UI bar is fixed at the bottom of the simulation region.
     * The height of the bar is UI_H.  */
    int UI_Y     = NY*scale;
    ...
    /* Sliders anchored to the bottom of the UI bar.  The frequency slider
     * sits above the speed slider.  X spans almost the full simulation width
     * with a small margin. */
    Slider s_freq  = { .x=8,
                       .y=UI_Y + UI_H - 60,
                       .w=NX*scale - 16,
                       .h=18,
                       .minv=0,.maxv=1,
                       .value=slider_from_freq(freq,FREQ_MIN,FREQ_MAX),
                       .dragging=0 };
    Slider s_speed = { .x=8,
                       .y=UI_Y + UI_H - 30,
                       .w=NX*scale - 16,
    ...
    while (running){
        ...
        while (SDL_PollEvent(&e)){
            ...
            if (e.type == SDL_KEYDOWN && e.key.repeat==0){
                ...
                /* Amp2 controls remapped from E/R to N/M to avoid collisions.  */
...
if (e.key.keysym.sym == SDLK_9){ cpml_apply_preset(2, dt); }
                 ...
                 * Remap from O/P to COMMA/PERIOD so that scope and screenshot keys remain unique. */
        ...
        if (show_legend){
            ...
            /* Replace the default top legend with a wrapped legend drawn in the bottom UI bar.
             * Build the legend text into a single buffer and draw it using draw_wrapped_panel().
             */
            {
                char legend_buf[4096]; legend_buf[0] = '\0';
                char lbuf[256];
                /* Basic run info */
                snprintf(lbuf,sizeof(lbuf), "freq: %.2f %s\n", disp, unit); strcat(legend_buf,lbuf);
                snprintf(lbuf,sizeof(lbuf), "Δ = %.4g m   dt = %.4g s\n", dx, dt); strcat(legend_buf,lbuf);
                snprintf(lbuf,sizeof(lbuf), "cpw (in/out): %.1f / %.1f\n", cpw_in, cpw_out); strcat(legend_buf,lbuf);
                snprintf(lbuf,sizeof(lbuf), "mode: %s  colorscale: %s%s%s  render:x%d\n", auto_rescale?"auto":"fixed", scmode,
                         under?"  under-res":"", clamped?"  auto-clamped":"", g_render_stride); strcat(legend_buf,lbuf);
                snprintf(lbuf,sizeof(lbuf), "steps/frame: %d   %s\n", steps_per_frame, paused?"paused":"running"); strcat(legend_buf,lbuf);
                /* Source amplitudes: annotate Amp2 if inactive */
                if (g_src[1].active) {
                    snprintf(lbuf,sizeof(lbuf), "sources: %d active   amp1=%.2f   amp2=%.2f   type: %s (toggle 1/2, cycle t)\n",
                             active_cnt, g_src[0].amp, g_src[1].amp, stype);
                } else {
                    snprintf(lbuf,sizeof(lbuf), "sources: %d active   amp1=%.2f   amp2=[off]   type: %s (toggle 1/2, cycle t)\n",
                             active_cnt, g_src[0].amp, stype);
                }
                strcat(legend_buf,lbuf);
                /* FPS and power flux */
                if (cpml_on) {
                    snprintf(lbuf,sizeof(lbuf), "fps: %.2f (inst), %.2f (avg)  powerflux=%.3e w/m\n", fps_inst, fps_avg, pflux_avg);
                } else {
                    snprintf(lbuf,sizeof(lbuf), "fps: %.2f (inst), %.2f (avg)  powerflux≈%.3e w/m (mur)\n", fps_inst, fps_avg, pflux_avg);
                }
                strcat(legend_buf,lbuf);
                /* Probe(s) and scope info */
                if (probe2_active) {
                    snprintf(lbuf,sizeof(lbuf), "probe1 (%d,%d) ez=%.3g   probe2 (%d,%d) ez=%.3g\n", probe_x, probe_y, Ez[probe_x][probe_y], probe2_x, probe2_y, Ez[probe2_x][probe2_y]);
                } else {
                    snprintf(lbuf,sizeof(lbuf), "probe1 (%d,%d) ez=%.3g\n", probe_x, probe_y, Ez[probe_x][probe_y]);
                }
                strcat(legend_buf,lbuf);
                snprintf(lbuf,sizeof(lbuf), "scope: %s  scale: %s  last: %.3g\n", scope.on?"on":"off", hold_scope?"hold":"auto", scope.last);
                strcat(legend_buf,lbuf);
                /* Material & boundary */
                snprintf(lbuf,sizeof(lbuf), "sigma: bg=%.3g s/m  block=%.3g s/m\n", SIGMA_BG, SIGMA_BLOCK); strcat(legend_buf,lbuf);
                if (cpml_on) {
                    snprintf(lbuf,sizeof(lbuf), "cpml: %s  n=%d  sigma=%.2f  kappa=%.2f  alpha=%.3f\n",
                             CPML_PRESETS[cpml_preset_idx].name, cpml_N, pml_sigma_max, pml_kappa_max, pml_alpha_max);
                } else {
                    snprintf(lbuf,sizeof(lbuf), "boundary: mur-1\n");
                }
                strcat(legend_buf,lbuf);
                /* Ports & sweep */
                if (sweep_on) {
                    snprintf(lbuf,sizeof(lbuf), "sweep: %d/%d  freq=%.2f mhz\n", sweep_idx+1, sweep_points,
                             (sweep_idx < sweep_points? (sweep_freqs[sweep_idx]/1e6) : 0.0));
                } else {
                    if (ports_on) {
                        if (s21_computed) snprintf(lbuf,sizeof(lbuf), "ports: on  last s21=%.3g\n", s21_amp);
                        else snprintf(lbuf,sizeof(lbuf), "ports: on\n");
                    } else {
                        snprintf(lbuf,sizeof(lbuf), "ports: off\n");
                    }
                }
                strcat(legend_buf,lbuf);
                /* Paint status */
                if (paint_mode) {
                    const char* pnames[] = {"off","pec","pmc","εr"};
                    if (paint_type == 3) {
                        snprintf(lbuf,sizeof(lbuf), "paint: %s  εr=%.2f\n", pnames[paint_type], paint_eps);
                    } else {
                        snprintf(lbuf,sizeof(lbuf), "paint: %s\n", pnames[paint_type]);
                    }
                } else {
                    snprintf(lbuf,sizeof(lbuf), "paint: off\n");
                }
                strcat(legend_buf,lbuf);
                /* Key help */
                strcat(legend_buf, "\nkeys:\n");
                strcat(legend_buf, "space pause  |  ↑/↓ freq  |  ←/→ steps  |  z render stride  |  r auto/fixed\n");
                strcat(legend_buf, "a autoscale  |  h hold color  |  [/] adjust color (hold)  |  j hold scope  |  \\ reset holds\n");
                strcat(legend_buf, "o scope on/off  |  k scope clear  |  y cpml  |  7/8/9 cpml presets  |  f dump fft  |  p screenshot\n");
                strcat(legend_buf, "1/2 toggle src  |  3 toggle probe2  |  q/w amp1 down/up  |  n/m amp2 down/up  |  t source type\n");
                strcat(legend_buf, "u paint on/off  |  i paint type (pec/pmc/εr)  |  ,/. change εr  |  s ports on/off  |  g s21 now  |  b sweep\n");
                /* Compute the legend area: leave room (~70 px) for sliders at the bottom */
                int legend_height = UI_H - 70;
                if (legend_height < 20) legend_height = UI_H;
                SDL_Rect legend_area = {8, UI_Y + 8, NX*scale - 16, legend_height};
