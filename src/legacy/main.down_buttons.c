...
/* ===================================================================== */
/* UI buttons for mouse-only interaction */
typedef struct {
...
static SDL_Texture* render_text(SDL_Renderer* ren, TTF_Font* font,
                                const char* s, SDL_Color col, int* w, int* h); /* fwd */

static void draw_button(SDL_Renderer* ren, TTF_Font* font, const UIButton* b){
    if (b->is_toggle && b->state && *(b->state)){
        SDL_SetRenderDrawColor(ren, 70,130,180,255);  /* ON */
    } else {
        SDL_SetRenderDrawColor(ren, 60,60,60,255);    /* OFF */
    }
    ...
    if (tt){
        SDL_Rect rr = { b->r.x + (b->r.w - tw)/2, b->r.y + (b->r.h - th)/2, tw, th };
...
static int ui_handle_click(int mx, int my){
    for (int i=0; i<ui_btn_count; ++i){
        ...
        if (mx >= b->r.x && mx < b->r.x + b->r.w && my >= b->r.y && my < b->r.y + b->r.h){
            if (b->is_toggle && b->state){ *(b->state) = !(*(b->state)); return UI_BTN_NONE; }
            return b->id;
        }
    }
    return UI_BTN_NONE;
...
int main(void){
    ...
    while (running){
        ...
        while (SDL_PollEvent(&e)){
            ...
            /* left-click:
            *  - if inside bottom UI bar: handle buttons (and stop)
            *  - else: paint when paint_mode ON, or drag nearest active source
            */
            if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                int mx = e.button.x, my = e.button.y;

                /* Bottom UI region short-circuit */
                if (my >= UI_Y) {
                    ...
                    if (id == UI_BTN_SCREENSHOT) {
                        ...
                        if (ports_on) {
                            ...
                            if (pf) { fprintf(pf, "# freq=%.9e\nS21_amp=%.9e\n", freq, new_s21);
                                    fclose(pf); fprintf(stderr, "Wrote ports_sparams.txt (S21=%.3g)\n", new_s21); }
                        } else {
                            fprintf(stderr, "Ports sampling is OFF; toggle with 's' or the button.\n");
                        }
                    }
                    /* handled UI click */
                    continue;
                ...
                /* Not UI area: paint/drag */
                if (paint_mode) {
                    int ix = clampi(mx / scale, 1, NX-2);
                    int iy = clampi(my / scale, 1, NY-2);
                    if (paint_type == 1) { /* PEC toggle */
                        tag_grid[ix][iy] = (tag_grid[ix][iy] == 1 ? 0 : 1);
                        epsr[ix][iy] = 1.0; sigma_map[ix][iy] = SIGMA_BG;
                    } else if (paint_type == 2) { /* PMC toggle */
                        tag_grid[ix][iy] = (tag_grid[ix][iy] == 2 ? 0 : 2);
                    } else if (paint_type == 3) { /* dielectric toggle */
                        if (fabs(epsr[ix][iy] - paint_eps) < 1e-6) {
                            epsr[ix][iy] = 1.0; sigma_map[ix][iy] = SIGMA_BG;
                        } else {
                            epsr[ix][iy] = paint_eps; sigma_map[ix][iy] = SIGMA_BG;
                        }
                        tag_grid[ix][iy] = 0;
                    }
                } else {
        ...
        /* Build buttons on bottom bar and draw them */
{
    ui_btn_count = 0;
    const int pad = 8;
    const int btn_h = 28;
    const int btn_w = 112;
    int bx = 12;
    int by = UI_Y + UI_H - btn_h - 8;

    UIButton b;

    /* Pause toggle */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="pause";
    b.is_toggle=1; b.state=&paused; b.id=UI_BTN_NONE; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Auto Δ toggle */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="auto Δ";
    b.is_toggle=1; b.state=&auto_rescale; b.id=UI_BTN_NONE; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Hold color toggle */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="hold color";
    b.is_toggle=1; b.state=&hold_color; b.id=UI_BTN_NONE; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* CPML toggle */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="cpml";
    b.is_toggle=1; b.state=&cpml_on; b.id=UI_BTN_NONE; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Ports toggle */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="ports";
    b.is_toggle=1; b.state=&ports_on; b.id=UI_BTN_NONE; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Paint toggle */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="paint";
    b.is_toggle=1; b.state=&paint_mode; b.id=UI_BTN_NONE; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Screenshot action */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="screenshot";
    b.is_toggle=0; b.state=NULL; b.id=UI_BTN_SCREENSHOT; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Clear action */
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="clear";
    b.is_toggle=0; b.state=NULL; b.id=UI_BTN_CLEAR; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;

    /* Optional: S21 now
    b.r = (SDL_Rect){bx, by, btn_w, btn_h}; b.label="S21 now";
    b.is_toggle=0; b.state=NULL; b.id=UI_BTN_S21_NOW; ui_btns[ui_btn_count++] = b; bx += btn_w + pad;
    */

    for (int bi=0; bi<ui_btn_count; ++bi) draw_button(ren, font, &ui_btns[bi]);

    /* React to toggles that require rebuilds */
    static int cpml_on_prev = 0, auto_prev = 0;
    if (cpml_on != cpml_on_prev) {
        cpml_on_prev = cpml_on;
        if (cpml_on){ cpml_build_coeffs(dt); cpml_zero_psi(); }
    }
    if (auto_rescale != auto_prev) {
        auto_prev = auto_rescale;
        if (auto_rescale) update_grid_for_freq(freq,&dx,&dy,&dt);
        else              { dx=BASE_DX; dy=BASE_DY; dt=cfl_dt(dx,dy); }
        cpml_build_coeffs(dt); if (cpml_on) cpml_zero_psi();
        clear_fields(); t=0;
