...
/*
 * Build the array of UI buttons for the side panel.  The boolean toggles are
 * passed as pointers so that the UI button system can modify the live state
 * variables directly.  Passing pointers also allows the button rendering code
 * to correctly reflect the ON/OFF status of each toggle.  Failure to pass
 * pointers (for example passing an int value) will result in the compiler
 * warning about converting an int to an int pointer and will cause the
 * buttons to modify a copy of the value rather than the real state variable.
 */
                               ...
                               int *cpml_on, int *ports_on, int *paint_mode)
{
    ...
    /* The toggles you asked for — short labels to fit nicely */
    /*
     * We deliberately pass the pointers directly (without casting to int*) so
     * that the ADD_TOGGLE macro stores the correct pointer in the UIButton
     * structure.  The UI button handler will dereference this pointer to
     * toggle the flag when clicked.  Casting to int* here would simply
     * convert the integer value (0 or 1) to a pointer, resulting in
     * undefined behaviour and compiler warnings.  See the function
     * signature above for details.
     */
...
/*
 * Boundary type enumeration.  The original code uses a boolean cpml_on to
 * toggle the Convolutional Perfectly Matched Layer (CPML) absorbing
 * boundary.  To support additional boundary conditions (for example
 * switching between Mur and CPML), define an explicit boundary_type
 * enumeration.  BOUNDARY_MUR corresponds to a first‑order Mur absorbing
 * condition, while BOUNDARY_CPML selects the CPML implementation.  The
 * boundary_type variable mirrors cpml_on for compatibility with older
 * code: when boundary_type == BOUNDARY_CPML, cpml_on should also be set.
 */
...
int main(void){
    ...
    int side_panel = 200; // room for colorbar + labels
    ...
    while (running){
        ...
        while (SDL_PollEvent(&e)){
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                /* --- side buttons click handling --- */
                {
                    ...
                    /*
                     * Pass the addresses of the state variables to build_side_buttons().  The
                     * function expects pointers so that toggles modify the actual variables.
                     */
                    build_side_buttons(sp_x, sp_y, sp_w, sp_h,
                                      &paused, &auto_rescale, &hold_color,
                                      &cpml_on, &ports_on, &paint_mode);
        ...
        /* right-side buttons panel (to the right of the colorbar) */
        int sp_x = cb_x + cb_w + 6;
        int sp_y = cb_y;
        int sp_w = NX*scale + side_panel - 6 - sp_x;
        int sp_h = cb_h;

        /* background for the buttons area */
        ...
        /* build and draw the buttons
         * Pass pointers to the state variables so the button system can
         * toggle them directly.  See build_side_buttons() for details.
         */
        build_side_buttons(sp_x, sp_y, sp_w, sp_h,
                           &paused, &auto_rescale, &hold_color,
                           &cpml_on, &ports_on, &paint_mode);
