...
static int choose_port_segment(int ny, int* out_y0, int* out_y1) {
    ...
    int y0 = clampi_local(ny / 4, safe_lo, safe_hi);
    int y1 = clampi_local((3 * ny) / 4, y0 + 1, ny - 1);
    if (y1 < y0) {
        y1 = y0;
    }
    ...
    if (span < kPortMinSegmentLen) {
        ...
        y0 = clampi_local(y0 - grow_lo, safe_lo, y0);
        y1 = clampi_local(y1 + grow_hi, y0, ny - 1);
        span = y1 - y0 + 1;
    }

    if (span < kPortMinSegmentLen) {
        y0 = safe_lo;
        y1 = clampi_local(y0 + kPortMinSegmentLen - 1, y0, ny - 1);
        span = y1 - y0 + 1;
    }
...
static int port_prepare_sample_range(const SimulationState* state, Port* port,
    ...
    int clamped_px = clampi_local(px, min_px, max_px);
    if (clamped_px != px) {
        port->x = clamped_px;
        px = clamped_px;
    }

    int y0 = clampi_local(port->y0, 0, state->ny - 1);
    int y1 = clampi_local(port->y1, y0, state->ny - 1);
    if (y1 < y0) {
        int tmp = y0;
        y0 = y1;
        y1 = tmp;
    ...
    if (span < kPortMinSegmentLen) {
        ...
        y0 = clampi_local(y0 - grow_bottom, 0, state->ny - 1);
        y1 = clampi_local(y1 + grow_top, y0, state->ny - 1);
        span = y1 - y0 + 1;
    }
