...
/*
 * Build the array of UI buttons within the supplied panel.  The boolean
 * toggles are driven by pointers to their respective state variables
 * rather than temporary values.  Passing addresses avoids storing
 * pointers to stack locals that can cause undefined behaviour when the
 * buttons are clicked.  The button positions are computed based on the
 * panel dimensions: the panel is subdivided into a grid of up to three
 * columns.  Action buttons have an ID associated with them; toggles
 * point directly at the boolean they control.
 */
static void build_side_buttons(int panel_x, int panel_y, int panel_w, int panel_h,
                               int *paused, int *auto_rescale, int *hold_color,
                               int *cpml_on, int *ports_on, int *paint_mode)
{
    ui_btn_count = 0;
    ...
    /* The toggles you asked for — short labels to fit nicely.  Note that
     * we pass the pointers directly so that button state changes will
     * modify the correct global variable instead of a copy. */
...
int main(void){
    ...
    int side_panel = 280; // enlarged room for colorbar + legend and controls
    ...
    while (running){
        ...
        while (SDL_PollEvent(&e)){
            ...
            if (e.type == SDL_MOUSEBUTTONDOWN && e.button.button == SDL_BUTTON_LEFT) {
                /* --- side buttons click handling --- */
                {
                    ...
                    /* Rebuild the same panel geometry here to ensure ui_btns have positions.  We
                     * subdivide the side panel into an upper information area and a lower
                     * button area.  Only the button area is passed to build_side_buttons. */
                    int sp_x = NX*scale + 6 + 14 + 6;                     /* cb_x + cb_w + 6 */
                    int sp_y = 6;
                    int sp_w = NX*scale + side_panel - 6 - sp_x;
                    int sp_h = NY*scale - 12;
                    /* Choose an information area height as a fraction of the side panel height.  */
        ...
        /* colorbar labels.  Display the maximum positive and negative values and a
         * zero marker next to the colour bar.  These labels were previously
         * commented out; they are now enabled and help the user interpret
         * the heatmap scale.  */
        {
        ...
        /* right-side panel (to the right of the colorbar).  We subdivide it into
         * a top section for simulation status, source information and boundary
         * settings, and a bottom section containing the interactive buttons.
         */
        ...
        /* Draw background for the entire side panel */
        ...
        /* Determine height for information panel (top portion).  Roughly 60% of
         * the side panel height is allocated to text; the remainder is for
         * buttons.  Adjust the fraction here to fine tune layout. */
        int info_h = (int)(0.6 * (double)sp_h);
        if (info_h < 100) info_h = (sp_h > 100 ? sp_h - 100 : sp_h);
        /* Construct a multi-line string with simulation parameters, sources and
         * boundary information.  We reuse the strings from the legend but
         * compress them into a single block of wrapped text.  */
        char info_text[2048];
        info_text[0] = '\0';
        {
            ...
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Δ = %.4g m   dt = %.4g s\n", dx, dt);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "CPW (in/out): %.1f / %.1f\n", cpw_in, cpw_out);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Mode: %s  ColorScale: %s%s%s  Render:x%d\n",
                    auto_rescale ? "AUTO" : "FIXED", scmode,
                    under   ? "  UNDER-RES"   : "",
                    clamped ? "  AUTO-CLAMPED": "",
                    g_render_stride);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Steps/frame: %d   %s\n", steps_per_frame, paused ? "PAUSED" : "RUNNING");
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Sources: %d active   Amp1=%.2f Amp2=%.2f   Type: %s\n",
                     active_cnt, g_src[0].amp, g_src[1].amp, stype);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "FPS: %.2f (inst), %.2f (avg)  PowerFlux=%.3e W/m\n", fps_inst, fps_avg, pflux_avg);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            /* Probe information */
            if (probe2_active) {
                snprintf(line, sizeof(line), "Probes: P1(%d,%d)=%.3g  P2(%d,%d)=%.3g\n",
                        probe_x, probe_y, Ez[probe_x][probe_y],
                        probe2_x, probe2_y, Ez[probe2_x][probe2_y]);
            } else {
                snprintf(line, sizeof(line), "Probe (%d,%d) Ez=%.3g (arb.)\n", probe_x, probe_y, Ez[probe_x][probe_y]);
            }
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Scope: %s  Scale: %s\n", scope.on ? "ON" : "OFF", hold_scope ? "HOLD" : "AUTO");
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Scope (last): %.3g\n", scope.last);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            snprintf(line, sizeof(line), "Sigma: bg=%.3g S/m  block=%.3g S/m\n", SIGMA_BG, SIGMA_BLOCK);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            /* Show current colour scale maximum.  This lets the user know the magnitude
             * represented by the red and blue extremes of the heatmap. */
            snprintf(line, sizeof(line), "Color scale ±%.3g\n", color_use_vmax);
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            /* Boundary info */
            if (cpml_on){
                snprintf(line, sizeof(line), "Boundary: CPML (%s)  N=%d  σ=%.2f  κ=%.2f  α=%.3f\n",
                    CPML_PRESETS[cpml_preset_idx].name, cpml_N, pml_sigma_max, pml_kappa_max, pml_alpha_max);
            } else {
                snprintf(line, sizeof(line), "Boundary: Mur-1\n");
            }
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            /* Ports and sweeps */
            if (sweep_on) {
                double frq = (sweep_idx < sweep_points? sweep_freqs[sweep_idx] : 0.0);
                snprintf(line, sizeof(line), "Sweep: %d/%d  f=%.2f MHz\n", sweep_idx+1, sweep_points, frq/1e6);
            } else {
                snprintf(line, sizeof(line), "Ports: %s  Last S21=%.3g\n", ports_on ? "ON" : "OFF", s21_amp);
            }
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            /* Painting */
            if (paint_mode) {
                ...
                if (paint_type == 3) {
                    snprintf(line, sizeof(line), "Paint: %s  εr=%.2f\n", pnames[paint_type], paint_eps);
                } else {
                    snprintf(line, sizeof(line), "Paint: %s\n", pnames[paint_type]);
                }
            } else {
                snprintf(line, sizeof(line), "Paint: OFF\n");
            }
            strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            /* List individual sources with properties */
            for (int ksrc=0; ksrc<MAX_SRC; ++ksrc){
                const char* snames[] = {"CW","Gauss","Ricker"};
                Source* s = &g_src[ksrc];
                snprintf(line, sizeof(line), "Src%d: %s  amp=%.2f  f=%.2f MHz  (%d,%d) %s\n",
                         ksrc+1, snames[s->type], s->amp, s->freq/1e6, s->ix, s->iy, s->active ? "ON" : "OFF");
                strncat(info_text, line, sizeof(info_text)-strlen(info_text)-1);
            }
        }
        /* Draw the information panel */
        SDL_Rect info_rect = { sp_x + 4, sp_y + 4, sp_w - 8, info_h - 8 };
        draw_wrapped_panel(ren, font, info_rect, info_text);

        /* Draw the buttons underneath the information panel */
        int btn_y = sp_y + info_h + 4;
        int btn_h = sp_h - info_h - 4;
        if (btn_h < 0) btn_h = 0;
        build_side_buttons(sp_x, btn_y, sp_w, btn_h,
